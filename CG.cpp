...
int p;
void moon();
void static_rocket();  //rockect yet to launch (178 line)
void rocket_to_cam_pos(); // rocket in mid air (3 scene)(417 line)
...
float i,j,count=0,count1=0,count3=0,flag=0,flag1=0,t=0,f=0,flag3=0,flag4=0,count4=0;
...
flag=1;
  if(flag==0)
  {
	...
	glColor3f(0.5,0.5,0.5);//stand object
	glBegin(GL_POLYGON);
	glVertex2f(130,10.0);
	glVertex2f(160,10.0);
	...
	glColor3f(0.0,0.74,0.6);//bonnet ,,head (white)
	glBegin(GL_POLYGON);//front
	glVertex2f(237.5,120.0);
	glVertex2f(262.5,120.0);
	glVertex2f(250,170.0);
	glEnd();
	glColor3f(0.0,0.0,1); //side jets (blue)
...
void rocket_to_cam_pos()   // rocket in mid air (2 scene)
{
	count++;
count3++;

for(i=0;i<=300;i++) //rocket speed through the sky
{
	...
	glColor3f(1,0,0.5);
	glBegin(GL_POLYGON);//core  body (puprle)
	...
	glEnd();
	glColor3f(0,0.0,1);   //side jets (blue)
	glBegin(GL_POLYGON);//left_side_top
	glVertex2f(237.5,120.0+i);
	...
	if((p%2)==0)
		...
		glEnd();
		p=p+1;
	for(j=0;j<=1000000;j++)  //rocket speed
		;
	glutSwapBuffers();
	glutPostRedisplay();
	glFlush();
...
void rocket_in_motion()    //rocket in space 
{
	count++;
...
for(i=195;i<=200;i++)
{
	...
	glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
	 ...
	 if(count<=130){
	glColor3f(1,0,0.5); 
	glBegin(GL_POLYGON);//core body(purple)
		glVertex2f(237.5,20.0+i);
		glVertex2f(262.5,20.0+i);
		glVertex2f(262.5,120.0+i);
		glVertex2f(237.5,120.0+i);
	...
	if(count<=110){
	glColor3f(1,0,0); //red color exhaust 
		glBegin(GL_POLYGON);//bottom_1_exhaust
	glVertex2f(237.5,20.0+i);
	glVertex2f(244.5,20.0+i);
	glVertex2f(241,0.0+i);
...
void mars(float radius) //moon position 
{
   ...
   {
      float degInRad = i*DEG2RAD;
      glVertex2f(460+f+cos(degInRad)*radius,500-t+(sin(degInRad))
...
void page()
{
	glColor3f(-0.75,0.75,0); //green
	glLineWidth(5);


	drawstring(5, 50, "Aakash Dayakar Suvarna ");
	drawstring(5,40,"4SF17CS001");
	drawstring(425, 50, "Pawan");
	drawstring(425,40,"4SF17CS103");
	...
	drawstring(180, 300, "INSTRUCTIONS TO LAUNCH");
